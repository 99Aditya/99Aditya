...
public class CanUpdate  extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			String query = "update candidatedetail set UserName=? ,Email_Id=? ,Mobile_Number=? ,Address =? ,Gender =? ,Marital_Status =?,Education =? where UserID= ? ";
		PreparedStatement ps=con.prepareStatement(query);
		  ...
		  RequestDispatcher rd = req.getRequestDispatcher("/candidate");
			rd.forward(req, resp);
			}
			else{
